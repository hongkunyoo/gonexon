...
.defines(function () {
	...
	EntityPlayer = ig.Entity.extend({
        animSheet: new ig.AnimationSheet( 'media/human2.png', 65, 80 ),
        size: {x: 32, y:80},
        offset: {x: 16, y: 0},
        flip: false,
        vel: {x: 20, y: 0},
        ...
        update: function () {
            // move left or right
 /*       	var spring = ig.game.getEntitiesByType( EntitySpring );
  
        	  if(this.distanceTo(spring) ==0){
        		  console.log("good");
            ...
            if(this.pos.x > ig.system.width/1.3){
        		this.vel.x = -GM.earthSpeed.x - 50;
        	}
			...
			arr.push(this.distanceTo(other));
				arr.sort();
