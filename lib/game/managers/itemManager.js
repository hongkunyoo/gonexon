...
//            			this.vel.x = GM.earthSpeed.x;
            			...
            			this.sound = new ig.Sound( 'media/sound/ogg/Pickup_Coin'+ran +'.ogg' ); 
        ...
        itemArrange: function (arrange_type, x, y) {
        	...
        	// Jump Guide Line
           	 case 1:
           	 		for(var i = 1; i < 8; i++){
           	 			if(i < 6)
           	 				item = this.generate(ItemManager.TYPE.STAR, x + 28*i, y);
           	 				//star = ig.game.spawnEntity(EntityStar, ig.system.width+28*i, ig.system.height/2);
           	 			if(i < 5)
           	 				item = this.generate(ItemManager.TYPE.STAR, x + 28*i + 28*5, y - 28*i);
           	 				//star = ig.game.spawnEntity(EntityStar, ig.system.width+28*i+28*5, ig.system.height/2-28*i);
           	 			else
           	 				item = this.generate(ItemManager.TYPE.STAR, x + 28*i + 28*5, y - 28*4 + 28*(i-4));
        	 ...
        	 // 3 X 5 Square Pattern
           	 case 2:
           	 		for(var i = 1; i < 6; i++){
           	 			item = this.generate(ItemManager.TYPE.STAR, x + 56, y - 28*i);
						//bob = ig.game.spawnEntity(EntityRice, ig.system.width+56, ig.system.height/2-28*i);
           	 			item = this.generate(ItemManager.TYPE.STAR, x, y - 28*i);
           	 			//star = ig.game.spawnEntity(EntityStar, ig.system.width, ig.system.height/2-28*i);
           	 			item = this.generate(ItemManager.TYPE.STAR, x + 28, y - 28*i);
        	 ...
        	 // 7 X 2 row Square Pattern
           	 case 3: 
           		 	for(var i = 1; i < 8; i++){
           		 		item = this.generate(ItemManager.TYPE.STAR, x + 28*i, y + 28);
           	...
           	// Heart Pattern	 	
           	 case 5:
           		 	for(var i = 1; i < 8; i++){
           		 		...
           		 		if( i == 2 || i == 6 ) {
           		 			item = this.generate(ItemManager.TYPE.STAR, x + 28*i, y - 28*2);
           		 		...
           		 		if ( i == 4 ){
           		 			item = this.generate(ItemManager.TYPE.STAR, x + 28*i, y);
           		 			//heart = ig.game.spawnEntity(EntityHeart, ig.system.width + 28*i, ig.system.height/4 );
           		 			item = this.generate(ItemManager.TYPE.STAR, x + 28*i, y + 28*3);
        	...
        	case 6 : 
        		for(var i = 1; i < 12; i++){
       	 			if(i < 7){
       	 				if ( i == 3 ){
       	 					for (var j = 0; j < 3 ; j ++ ) 
       	 						item = this.generate(ItemManager.TYPE.A_SCORE, x + 14*i + 28*5 + 28*j, y - 28*i);
       	 					//aplus = ig.game.spawnEntity(EntityA, ig.system.width + 14*i + 28*5 + 28*j , ig.system.height/2-28*i);      	 					      	 				    
       	 				}
       	 				else
       	 					item = this.generate(ItemManager.TYPE.A_SCORE, x + 14*i + 28*5, y - 28*i);
       	 					//aplus = ig.game.spawnEntity(EntityA, ig.system.width+14*i+28*5, ig.system.height/2-28*i);
       	 			}
       	 			else
       	 				item = this.generate(ItemManager.TYPE.A_SCORE, x + 14*i + 28*5, y - 28*6 + 28*(i-6));
        	...
        	case 7 :
        		for(var i = 1 ; i < 6 ; i ++){
        			item = this.generate(ItemManager.TYPE.F_SCORE, x + 28, y/2 - 28*i);
        			//f = ig.game.spawnEntity(EntityF, ig.system.width + 28 , ig.system.heigth/2 - 28*i );
        			
        			if( i == 3 ) {
        				for (var j = 1 ; j < 5 ; j ++ )
        					item = this.generate(ItemManager.TYPE.F_SCORE, x + 28*j, y/2 - 28*i);
