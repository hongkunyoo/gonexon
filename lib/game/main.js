...
MyGame = ig.Game.extend({
	...
	init: function() {
		...
		var block = this.spawnEntity(EntityBlock, 260, ig.system.height-10);
	...
	draw: function() {
	    ...
	    this.font.draw("Timer",255,5);
	    var elapse = this.levelTimer.delta();
	    this.font.draw(elapse.toFixed(1),295,5);
	    
	    this.font.draw("Coins",190,5);
	    this.font.draw(GM.COIN,220,5);
	    /*
	    if(this.showStats){
...
GameOverScreen = ig.Game.extend({
	...
	draw: function() {
		...
		score = GM.COIN * MyGame.levelTimer;//코인*알파+시간*베타
		
		this.instructText.draw('Total time: '+MyGame.levelTimer, x, y+30, ig.Font.ALIGN.CENTER);
		this.instructText.draw('Total Coin: '+GM.COIN, x, y+40, ig.Font.ALIGN.CENTER);
		this.instruxtText.draw('Total Score: '+ score, x, y+50, ig.Font.ALIGN.CENTER);
		this.instruxtText.draw('Press Spacebar To Continue.', x, ig.system.height,ig.Font.ALIGN.CENTER);
	}
});
