...
//        	IM.fetchAndMakeItem(this.levelData["path"][this.levelSceneCount]["item"]);
    		...
    		if (blockData != undefined) {
    			BM.getFromPool(BlockManager.TYPE[blockData.type], 
        				this.currentEarth.pos.x + blockData.x ,
        				this.currentEarth.pos.y - this.currentEarth.size.y - blockData.y);
    		}
    		
    		
    		var monsterData = this.levelData["path"][this.levelSceneCount]["monster"];
    		if (monsterData != undefined) {
    			IM.generate(ItemManager.TYPE[monsterData.type], this.currentEarth.pos.x + monsterData.x,
        				this.currentEarth.pos.y - this.currentEarth.size.y - monsterData.y);
    		}
    		...
    		if (itemData != undefined) {
    			if (ItemManager.ARRANGE_TYPE[itemData.type] != undefined) {
    		
    				IM.itemArrange(ItemManager.ARRANGE_TYPE[itemData.type], this.currentEarth.pos.x + itemData.x, 
    						this.currentEarth.pos.y - this.currentEarth.size.y - (itemData.y-100));
        		} else {
        			console.log(ItemManager.TYPE[itemData.type]);
        			IM.generate(ItemManager.TYPE[itemData.type], itemData.x + this.currentEarth.pos.x, 
        					this.currentEarth.pos.y - this.currentEarth.size.y - (itemData.y));
        		}
    		}
