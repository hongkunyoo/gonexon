...
MyGame = ig.Game.extend({
	...
	update: function () {
		...
		if (ig.input.pressed('test')) {
			//IM.generate(ItemManager.TYPE.SPRING, 700, 100);
			//IM.itemArrange(6, 760, 200);
//			JUMP_GUIDE_LINE: 1,
//	    	SQUARE_3_5: 2,
//	    	SQUARE_7_2: 3,
//	    	HEART: 5,
//	    	A_PATTERN: 6,
//	    	F_PATTERN: 7
			//IM.itemArrange(ItemManager.ARRANGE_TYPE.A_PATTERN, 500, 350);
			GM.addHeart();
			//ig.game.spawnEntity(EntityCanItem,500,200);
			//IM.generate(ItemManager.TYPE.TWINKLE, 500,200);
		}
	},
...
StartScreen = ig.Game.extend({
	...
	update: function() {
		if(ig.input.pressed ('start')){
			ig.system.setGame(ManualScreen);
		}
		this.parent();
	},
...
ManualScreen = ig.Game.extend({
    instructText: new ig.Font( 'media/04b03.font.png' ),
    Manual: new ig.Image('media/notice.png'),
    ...
    draw: function() {
       ...
       this.Manual.draw(x - (this.Manual.width * .5), y-200);
      
       x = ig.system.width/4-100;
       y = y-100;
       this.instructText.draw('Space Bar : Start/Continue', x, y, ig.Font.ALIGN.LEFT);
       this.instructText.draw('Right Arrow : Dash', x, y+30, ig.Font.ALIGN.LEFT);
       this.instructText.draw('Key X : Jump', x, y+60, ig.Font.ALIGN.LEFT);
       this.instructText.draw('Key X*2 : Double Jump', x, y+90, ig.Font.ALIGN.LEFT);
       this.instructText.draw('Key Z : Shoot', x, y+120, ig.Font.ALIGN.LEFT);
