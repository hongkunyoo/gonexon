	...
	ItemManager = ig.Class.extend({
        ...
        itemArrange: function(num){
        	switch (num) {
        	 case 0:
           	 ...
           	 case 1:
           	 		for(i = 1; i < 8; i++){
           	 			if(i<6)
           	 				coin = ig.game.spawnEntity(EntityCoin, ig.system.width+27*i , ig.system.height/2);
           	 			if(i<5)
           	 				coin = ig.game.spawnEntity(EntityCoin, ig.system.width+27*i+27*5, ig.system.height/2-27*i);
           	 			else
           	 				coin = ig.game.spawnEntity(EntityCoin, ig.system.width+27*i+27*5, ig.system.height/2-27*4+27*(i-4));
           	 		}
        			break;
           	 case 2:
           	 		for(i = 1; i < 5; i++){
        	...
        	////////////////////빵보!
           	 case 3: 
           		 	for(i = 1; i < 8; i++){
           		 		coin = ig.game.spawnEntity(EntityCoin, ig.system.width + 27*i , ig.system.height/2 + 27);
           		 		coin = ig.game.spawnEntity(EntityCoin, ig.system.width + 27*i , ig.system.height/2);
           		 	}
           		 	break;
