	...
	EntitystarManager = ig.Entity.extend({
		...
		check: function (other) {
			if( this.pos.y< ig.system.height/2){
				...
				else if(this.pos.x< ig.system.width + 11*28 && this.pos.x>ig.system.width+20){
					console.log("0");
					IM.canPutFront = 0;
				}
				else if(this.pos.x > ig.system.width + 11*28){
					IM.canPutFront = 0;
				}
				else{
					IM.canPutFront = 0;
        ...
        update: function () {
			...
			if(this.setStarTimer.delta()>0){
				...
				check3 = ig.game.spawnEntity(EntitystarManager, ig.system.width+6*28, 0);
					console.log("canPutFront = " + this.canPutFront +" canPutMiddle = " + this.canPutMiddle +" canPutBack = " + this.canPutBack );
				if(this.canPutFront != 0 && this.canPutBack != 0 && this.canPutMiddle != 0){
					this.itemArrange( Math.floor((Math.random() * 1000) % 6+1));
					
					console.log("이러니까 출력하지ㅋㅋ canPutFront = " + this.canPutFront +" canPutMiddle = " + this.canPutMiddle +" canPutBack = " + this.canPutBack );
				}
				else
					
					console.log("이래서 출력안되는거에용!!! canPutFront = " + this.canPutFront +" canPutMiddle = " + this.canPutMiddle +" canPutBack = " + this.canPutBack );
			}
